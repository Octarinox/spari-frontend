export const calculateQueueAverage = (
   ...
   Object.keys(result).forEach(intervalKey => {
      ...
      const totalSalaro1 = branchIds.reduce((sum, branchId) => {
         const branch = data.find(item => item.branch_id === branchId);
         return sum + (branch?.detect.Salaro1?.person || 0);
      }, 0);

      const totalSalaro2 = branchIds.reduce((sum, branchId) => {
         const branch = data.find(item => item.branch_id === branchId);
      ...
      const finalAverage = averageSalaro1 + averageSalaro2 / 2;
